...
public class Axe : PowerUp
{
    ...
    Vector2 ThrowDirection()
    {
        ...
        Vector2 lookDir = mousePos - Player.Instance.transform.position;
        return lookDir;
    }
    private void Start()
    {
    ...
    private void Update() 
    {
        if (!isReturn && Vector3.Distance(transform.position, Player.Instance.transform.position) >= maxDistance) 
        {
            isReturn = true;
        }
        if (isReturn) 
        {
            Vector2 returnDir = (Player.Instance.transform.position - transform.position).normalized;
